...
$$\mathit{confs} = (h_1, \ctxts_1), \ldots, (h_n, \ctxts_n)$$
taki że $h_1 = h$, $\ctxts_1 = \ctxts$ oraz $(h_i, \ctxts_i) \rightarrow (h_{i+1}, \ctxts_{i+1})$ dla $1 \leq i < n$.
...
$(h, e) \eval{confs} (h_n, A, l)$ lub po prostu $(h, e) \eval{} (h_n, A, l)$, jeśli dokładny ciąg
konfiguracji nie jest istotny.
...
wolnej $x$ w~$E$, czyli $E[/env] = E[l_1/x_1, \ldots, l_n/x_n]$, gdzie $\dom{env} = \{x_1, \ldots, x_n\}$, a
$l_i = env(x_i)$ dla $i = 1, \ldots, n$.
...
gdzie:
\begin{align*}
& \alloc(h_1, \ctxts_1', C)=(l_0, h) \\
& \fields(C) = x_1, \dots, x_k \\
& o=\emptyclass{C}\{x_1 \mapsto  l_1,\ldots,x_k~\mapsto~l_k\} \\
& h_{1,n} = h\{l_0 \mapsto o\}
\end{align*}
...
\begin{proof} (twierdzenia o zależności ewaluacji od zmiennych wolnych)
    ...
    Pokażę, że istnieją $\hbase_{1, n}$, $\hbase_{2, n}$, $\mathit{confs}_2$, $h_{2, n}, l_2, f$,
...
zachowanie wszystkich własności z tezy lematu, co wygląda analogicznie jak w dowodzie
...
elementy sterty i środowiska, które odpowiadają zmiennym wolnym w tym termie.
...
się wyprowadzić przy użyciu reguł dowodzenia i niezmienników będących wcześniej na liście.
...
jest wyprowadzalny dla każdego $E$ będącego ciałem metody $m$ w klasie $C$ lub jej podklasie i
...
odpowiednika w podklasach. Z jednej strony ogranicza to zakres wnioskowania o metodach,
...
zmiennej do środowiska nie zmienia spełnialności tego termu przez stertę (Lemat \ref{lem:fresh_var_equiv}).
...
Z tego powodu kwantyfikator $\exists$ został w logice ograniczony do termów najwyższego poziomu, tak
...
Pozostaje więc pokazać, że semantyka termu dla obu izomorficznych stert jest taka sama.
...
dla nich fragmentów sterty, a następnie rozszerzanie tych fragmentów do
...
Opisuje ona zachowanie ewaluacji podczas wywołania metody $m$ na obiekcie klasy $C$,
...
to sterta powstała po ewaluacji wyrażenia $E_1$, a zatem bez założenia o trwałości mogłaby
ona nie spełniać $S$, co uniemożliwiłoby zastosowanie założenia indukcyjnego.
...
o tym, że jeśli z \texttt{p} można udowodnić \texttt{q}, a z \texttt{q} można udowodnić
...
i środowisku typów $\Gamma$ term $S$ wynika semantycznie z termu $P$, jeśli
dla dowolnej sterty $h$ i środowiska $env$, takich że $env$ zgadza się z $\Gamma$ na $h$,
jeśli $h$ spełnia $P$ w $env$, to $h$ spełnia również $S$ w $env$:
\begin{verbatim}
Definition JFISemanticallyImplies gamma P S (CC : JFProgram) :=
  forall env this h,
...
przez powiedzenie, że jeśli $\Gamma | P \vdash S$, to $S$ wynika semantycznie z $P$
w środowisku $\Gamma$ :
\begin{verbatim}
Theorem JFISoundness : forall gamma decls P S,
  JFIProves decls gamma P S ->
  JFISemanticallyImplies gamma P S (JFIDeclsProg decls). 
\end{verbatim}

Twierdzenie to jest następnie dowodzone przez indukcję po budowie drzewa dowodu
dla \texttt{JFIProves decls gamma P S} zgodnie z opisem w rozdziale \ref{chap:soundness}.
