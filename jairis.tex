...
Dodatkowo zakładamy, że wszystkie zmienne wolne występujące w termach w danych osądzie muszą być
...
Ponieważ jednak Jafun jest silnie typowany, więc w ścisłym sformułowaniu tego twierdzenia potrzebne
jest jeszcze założenie o odpowiednim otypowaniu obiektów na stercie.
...
\begin{theorem}[O poprawności logiki separacji dla języka Jafun]{\ } \\
  ...
  Wtedy dla każdej sterty $h$ i środowiska $\mathit{env}$, takich że $\mathit{env}$ zgadza się z $\Gamma$
  na stercie $h$, jeśli  $\[ P \]_{h, env} \triangleq \top$, to również $\[ Q \]_{h, env} \triangleq \top$.
  \qed
\end{theorem}

Dowód tego twierdzenia przebiega przez indukcję po budowie drzewa dowodu dla $\Gamma | P \vdash Q$.
...
To znaczy jeśli $x \not\in FV(P)$, to $\[ P \]_{h, env} \triangleq \[ P \]_{h, env[x \mapsto l]}$.
